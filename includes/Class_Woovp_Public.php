...
class Class_Woovp_Public {
        ...
        if (!is_admin() && get_post_type($post_id) === 'product_variation') {
            ...
            if (!empty($attributes)) {
                ...
                if (isset($attributes[$flavour_key])) {
                    ...
                    $term = get_term_by('slug', $flavour_value, 'pa_flavour');
                    if ($term) {
                        $title = ucwords(str_replace('-', ' ', $flavour_value)) . ' - ' . $title;
                    }
                }
            }
        }
    ...
    public function display_color_variations_as_individual_products($query) {
        if ((is_shop() || is_product_category() || is_product_tag()) && $query->is_main_query() && !is_admin()) {
    ...
    /**
     * Initializes the Class_Woovp_Public class.
     */
    public static function init() {
        $class = new self();
        add_action('plugins_loaded', [$class, 'setup']);
    }

    /**
     * Sets up the Class_Woovp_Public class.
     */
